...
export function KanbanBoard({ boards, cards, apiUrl }: Props) {
    ...
    function getDraggingTaskData(taskId: UniqueIdentifier, columnId: UniqueIdentifier) {
        const tasksInColumn = tasks.filter((task) => task.kanbanBoard.id === columnId);
    ...
    const announcements: Announcements = {
        onDragStart({ active }) {
            ...
            if (active.data.current?.type === "Column") {
                ...
                pickedUpTaskColumn.current = active.data.current.task.kanbanBoard.id;
                const { tasksInColumn, taskPosition, column } = getDraggingTaskData(
                    active.id,
                    pickedUpTaskColumn.current
                );
                return `Picked up Task ${active.data.current.task.kanbanBoard.id
        ...
        onDragOver({ active, over }) {
            ...
            if (
                ...
                const { tasksInColumn, taskPosition, column } = getDraggingTaskData(
                    over.id,
                    over.data.current.task.kanbanBoard.id
                );
                if (over.data.current.task.kanbanBoard.id !== pickedUpTaskColumn.current) {
                ...
                <SortableContext items={columnsId}>
                    {columns.map((col) => (
                        <BoardColumn
                            ...
                            key={col.id}
                            column={col}
                            tasks={tasks.filter((task) => task.kanbanBoard.id === col.id)}
                        />
                    ))}
                </SortableContext>
            </BoardContainer>
                    ...
                    <DragOverlay>
                        {activeColumn && (
                            <BoardColumn
                                ...
                                column={activeColumn}
                                tasks={tasks.filter(
                                    (task) => task.kanbanBoard.id === activeColumn.id
                                )}
                            />
                        )}
    ...
    function onDragOver(event: DragOverEvent) {
        ...
        if (isActiveATask && isOverATask) {
            setTasks((tasks) => {
                ...
                if (
                    activeTask &&
                    overTask &&
                    activeTask.kanbanBoard.id !== overTask.kanbanBoard.id
                ) {
                    activeTask.kanbanBoard.id = overTask.kanbanBoard.id;
        ...
        if (isActiveATask && isOverAColumn) {
            setTasks((tasks) => {
                ...
                const activeTask = tasks[activeIndex];
                if (activeTask) {
                    activeTask.kanbanBoard.id = overId as UniqueIdentifier;
